...
Autonomous job-application engine for SWE/ML roles. Pipeline: **discover** jobs across aggregators + ATS APIs + search dorks → **enrich** with full descriptions via tiered cascade → **tailor** resume per job via Codex API → *(future)* apply via headless browser → *(future)* contact discovery → *(future)* cold outreach.
...
- Tailoring (`src/applyd/tailor/`) — Codex API with prompt caching, tectonic PDF compile, structural validator, structured JSON metadata output. Multi-tenant entry point: `tailor_for_user(user_id, job_id)` in `tailor/saas.py`. CLI routed through this path.
...
### Tailoring
- **Codex Sonnet 4.6 default.** Prompt caching is mature; cached reads ~10× cheaper. Writing quality on constrained rewrites is strong. Model swappable via `TailorClient`.
...
python -m applyd.apply.runner <job_id> [--model <slug>] [--profile <path>] [--test-mode true|false]
                         │
                         │ OpenAI SDK ──► OpenRouter (default: anthropic/Codex-sonnet-4-6)
                         ▼
...
- `OPENROUTER_API_KEY` — direct apply runner's LLM gateway (default model: `anthropic/Codex-sonnet-4-6`)
...
- Disclose bias: when recommending Anthropic products (Codex, Codex, Codex Agent SDK), explicitly note you're made by Anthropic.
